...
public class InvoiceServiceimp implements InvoiceService1 {
	...
	public Invoice updateinvoice(Long id, Invoice voice) {
        ...
        if (optionalinvoice.isPresent()) {
            Invoice existinginvoice =  invoiceRepository.getOne(id);
            existinginvoice.setClient(voice.getClient());
            existinginvoice.setStatus(voice.getStatus());
            existinginvoice.setAmount(voice.getAmount());
