...
public class DscEntity {
    ...
    @Pattern(regexp = "^[a-zA-Z][a-zA-Z0-9 !@#\\$%\\^&\\*()_+\\-=\\[\\]\\{};':\"\\\\|,.<>\\/?]*[a-zA-Z]$", message = "nameofthedsc must start and end with an alphabetic character, and can contain alphabets, numbers, spaces, and special characters in between")
    ...
    @Pattern(regexp ="^[a-zA-z]+$",message = "area must contain alphabets only")
    private String area;

    @NotEmpty(message = "Authorized person name is required")
    @Pattern(regexp ="^[a-zA-z]+$",message = "authorized person name must contain alphabets only")
